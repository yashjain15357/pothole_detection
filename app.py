...
def load_google_oauth_credentials():
    """Load Google OAuth client credentials from environment variables or a local JSON file."""
...
def register_google_oauth():
    """Register the Google OAuth client only when credentials are available."""
...
def smtp_config_ready():
    """Check whether SMTP credentials are available in the environment or config file."""
...
def extract_location_from_text(report_text: str):
    """Extract latitude/longitude from plain-text report content if present."""
    if not report_text:
        return None

    lat_match = re.search(r'Latitude\s*:\s*([-+]?\d+(?:\.\d+)?)', report_text, re.IGNORECASE)
    lng_match = re.search(r'Longitude\s*:\s*([-+]?\d+(?:\.\d+)?)', report_text, re.IGNORECASE)
...
def get_location_for_report(report, report_text=None):
    """Get location for a report from text, paired JSON, or location history files."""
    location = extract_location_from_text(report_text or '')
    if location:
        return location

    # 1) Try paired JSON next to the report TXT path (supports escaped/Windows paths)
    report_path = (report or {}).get('path', '')
    resolved_path = resolve_report_file(report_path)
    if resolved_path:
        candidate_json = resolved_path.with_suffix('.json')
        if candidate_json.exists():
            try:
                with open(candidate_json, 'r', encoding='utf-8') as jf:
                    payload = json.load(jf)
                loc = payload.get('location', {}) if isinstance(payload, dict) else {}
    ...
    # 2) Try location history reports generated by map selection
    report_type = (report or {}).get('report_type', '')
    report_name = str((report or {}).get('name', '')).strip()

    location_dirs = []
    if report_type == 'image':
        location_dirs = [Path('IN_image/locations'), Path('/tmp/IN_image/locations')]
    elif report_type == 'video':
        location_dirs = [Path('IN_vedio/locations'), Path('/tmp/IN_vedio/locations')]
    ...
    for location_dir in location_dirs:
        ...
        try:
            json_files = sorted(location_dir.glob('*.json'), reverse=True)
            # Prefer matching filename first; otherwise use latest available.
            for file_path in json_files:
                try:
                    ...
                    payload_name = str(payload.get('filename', '')).strip()
                    if report_name and payload_name and payload_name != report_name:
                        continue
            ...
            # Fallback: use latest location entry if no strict filename match found
            if json_files:
                try:
...
def upload_image():
    ...
    try:
        # Save the uploaded file locally before running model inference.
...
def download_report(report_path):
    """Download a report from disk, with compatibility fallbacks for older Flask versions."""
    ...
    if size == 0:
        print("Report file size is 0 bytes — returning error instead of empty download")
...
def download_report_pdf(report_id):
    ...
    try:
        ...
        if not report_text and report.get('path'):
            try:
                p = resolve_report_file(report.get('path'))
                if p and p.exists():
        ...
        # Ensure location appears in PDF when available from any report source.
        location = get_location_for_report(report, report_text)
        if location and not re.search(r'Latitude\s*:', report_text, re.IGNORECASE):
            report_text = (
                "LOCATION\n"
...
def save_camera_report():
    ...
    try:
        ...
        try:
            ...
            print(f"Failed to save report blob to DB: {e}")
...
def send_report_email():
    """Send a saved report file by SMTP email attachment."""
    try:
        data = request.get_json() or {}
